...
parser.add_argument('--outf', default='./result/train/mnist', help='folder to output images and model checkpoints')
...
input = torch.Tensor(opt.batchsize, nc, opt.imagesize, opt.imagesize)
input_tile = torch.Tensor(opt.batchsize * jcbSize, nc, opt.imagesize, opt.imagesize)
regress_img = torch.Tensor(opt.batchsize, nc, opt.imagesize, opt.imagesize)
noise = torch.Tensor(opt.batchsize, nz, 1, 1)
pos_noise = torch.Tensor(opt.batchsize * jcbSize, nz, 1, 1)
zero_noise = torch.Tensor(opt.batchsize, nz, 1, 1)
fixed_noise = torch.Tensor(opt.batchsize, nz, 1, 1).normal_(0, opt.var)
label = torch.Tensor(opt.batchsize)
eye_label = torch.Tensor(opt.batchsize, jcbSize, jcbSize)
eye_nz = torch.Tensor(opt.batchsize, jcbSize, nz)
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        eye_label.resize_(batch_size, jcbSize, jcbSize).copy_(torch.eye(jcbSize))
        eye_labelv = Variable(eye_label)

        eye_nz.resize_(batch_size, jcbSize, nz).copy_(torch.eye(nz)[torch.randperm(nz)[:jcbSize]])
        ...
        if i % 20 == 0:
            print('Epoch: [%2d/%2d] [%4d/%4d] time: %4.4f, Loss_D: %.4f Loss_G: %.4f L1_Loss: %.4f Orth_Loss: %.4f'
                  % (epoch+1, opt.niter, i, len(dataloader), time.time()-start_time,
                     errD.item(), errG.item(), errL1.item(), errOrth.item()))
        if i % 100 == 0:
            netG.eval()
            vutils.save_image(inputv.data,
                              '%s/real_samples_epoch_%03d_%3d.png' % (opt.outf, epoch, i),
                              normalize=True)
            fake = netG(inputv, fixed_noise)
            vutils.save_image(fake.data,
                              '%s/fake_samples_epoch_%03d_%3d.png' % (opt.outf, epoch, i),
                              normalize=True)
            recons = netG(inputv, zero_noisev)
            vutils.save_image(recons.data,
                              '%s/reconstruction_epoch_%03d_%3d.png' % (opt.outf, epoch, i),
                              normalize=True)
            netG.train(True)
